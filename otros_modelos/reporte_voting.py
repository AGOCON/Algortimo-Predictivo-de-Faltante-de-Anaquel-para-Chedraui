...
seed = 0
vc1 = RandomForestClassifier(random_state=seed)
vc2 = AdaBoostClassifier()
vs = VotingClassifier(estimators=[('RF', vc1), ('Ada', vc2)],
                        voting='soft', weights=[1,1])


vs.fit(X_train, y_train)
vs.predict(X_test)
acc_train = vs.score(X_train,y_train)
acc_test = vs.score(X_test,y_test)
...
y_pred_proba = vs.predict_proba(X_val)[:,1]
