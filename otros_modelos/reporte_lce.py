...
acc_test = lce.score(X_test,y_test)
...
with open('Reporte_General_Voting.txt','w') as f:
    f.write(f'#####Calificaciones obtenidas por el algoritmo de local cascade ensemble ########## \n')
...
plt.xlabel('fpr')
plt.ylabel('tpr')
plt.title('ROC curve for the LCE Classifier')
plt.show()
plt.savefig('LCE_ROC_curve.png')
